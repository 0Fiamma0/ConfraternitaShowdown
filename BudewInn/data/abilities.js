...
let BattleAbilities = {
	...
	update: {
		desc: "Abilità legame di Cinnamon",
		shortDesc: "Abilità legame di Cinnamon",
		onStart(pokemon) {
			this.boost({atk: 1}, pokemon);
		},
		onPrepareHit(source, target, move) {
			...
			if (type && type !== '???' && source.getTypes().join() !== type) {
				if (!source.setType(type)) return;
				this.add('-start', source, 'typechange', type, '[from] ability: Update');
			}
		},
		id: "Update",
		name: "Update",
	...
	rampage: {
		desc: "Abilità legame di Hydreigon -di Hart-",
		shortDesc: "Abilità legame di Hydreigon -di Hart-",
		onDamagingHit(damage, target, source, effect) {
			this.boost({spa: 1});
		},
		id: "rampage",
	...
	snowstorm: {
		shortDesc: "Abilità legame di Artemisia",
		onStart(source) {
			this.field.setWeather('hail');
	...
	determination: {
		desc: "Abilità legame di Midir",
		shortDesc: "Abilità legame di Midir",
		onStart(pokemon) {
			let activated = false;
			for (const target of pokemon.side.foe.active) {
				if (!target || !this.isAdjacent(target, pokemon)) continue;
				if (!activated) {
					this.add('-ability', pokemon, 'Determination', 'boost');
					activated = true;
				}
				if (target.volatiles['substitute']) {
					this.add('-immune', target);
